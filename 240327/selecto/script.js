...
const tableBody = document.querySelector(".shopinfo_table > table > tbody");
...
const renderPagination = (data) => {
  ...
  if (currentPageGroup < Math.ceil(totalPages)) {
...
cityOption.addEventListener("change", () => {
  ...
  const seletedCity = cityOption.value;
  if (seletedCity !== "none") {
    stateOption.innerHTML = "<option>구/군 선택</option>";
    if (cityInfo[seletedCity]) {
      cityInfo[seletedCity].forEach((cityItem) => {
        const option = document.createElement("option");
  ...
  const matchedCityInfo = storeInfo.filter(
    (city) => city.main_area === seletedCity
  );
  tableBody.innerHTML = "";
  if (matchedCityInfo.length > 0) {
    renderData(matchedCityInfo);
